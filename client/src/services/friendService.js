...
export const friendService = {
  sendFriendRequest: async (recipientUsername) => {
    try {
      const response = await api.post(
        `/api/friend-requests/send?recipientUsername=${recipientUsername}`,
        {},
        {
  ...
  declineFriendRequest: async (requestId) => {
    try {
      const response = await api.post(
        `/api/friend-requests/decline/${requestId}`,
        {},
        {
          headers: getAuthHeader(),
        }
  ...
  getFriends: async () => {
    ...
    try {
      const response = await api.get(`/api/friend-requests/friends/${user.username}`, {
        headers: getAuthHeader(),
      });
      return response.data;
    } catch (error) {
  ...
  blockUser: async (blockedUsername) => {
    try {
      const response = await api.post(
        `/api/friend-requests/block?blockedUsername=${blockedUsername}`,
        {},
        {
          headers: getAuthHeader(),
        }
  ...
  isBlocked: async (otherUsername) => {
    try {
      const response = await api.get(
        `/api/friend-requests/is-blocked?otherUsername=${otherUsername}`,
        {
          headers: getAuthHeader(),
        }
      );
  ...
  areFriends: async (otherUsername) => {
    const response = await api.get(
      `/api/friend-requests/are-friends?otherUsername=${otherUsername}`,
      {
        headers: getAuthHeader(),
      }
    );
  ...
  isFriendRequestPending: async (otherUsername) => {
    const response = await api.get(
      `/api/friend-requests/is-pending?otherUsername=${otherUsername}`,
      {
        headers: getAuthHeader(),
      }
    );
