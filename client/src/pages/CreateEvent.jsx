  ...
  const handleImageChange = (event) => {
    ...
    if (file) {
      ...
      if (!validTypes.includes(file.type)) {
        setErrors({
          ...errors,
          image: 'Lütfen geçerli bir görsel formatı seçin (JPEG, PNG, GIF, WEBP, BMP)'
        });
        return;
      }
    ...
    try {
      ...
      if (formData.image) {
        ...
        if (!validTypes.includes(formData.image.type)) {
          throw new Error('Sadece JPEG/JPG, PNG, GIF ve WEBP formatları desteklenmektedir.');
        }
      ...
      formDataToSend.append('startTime', formData.startTime.format('YYYY-MM-DDTHH:mm:ss'));
      formDataToSend.append('endTime', formData.endTime.format('YYYY-MM-DDTHH:mm:ss'));
